...
touch UI. Audio playback is handled by Raspotify/librespot; the web app reads
local librespot playback events for Now Playing state and uses the Spotify Web
API only as a fallback and for optional controls.
...
- Local Raspotify/librespot Now Playing metadata:
  - album art
  - artist
  - track title
  - album
...
UI can read metadata from local Raspotify events once playback starts. Spotify
API credentials are still recommended for fallback metadata and required for
...
/opt/spotify-now-playing/
  ...
  raspotify-event-writer.py

/etc/spotify-now-playing.env
/etc/systemd/system/spotify-now-playing.service
/etc/raspotify/conf
/run/raspotify/now-playing.json
/usr/local/bin/spotify-now-playing-kiosk
```

`/run/raspotify/now-playing.json` is runtime state generated by librespot's
event hook. It is recreated automatically when playback starts.
...
sudo cat /run/raspotify/now-playing.json
sudo systemctl restart spotify-now-playing.service
```
...
### Screen Says Nothing Playing While Audio Plays

Check that Raspotify is configured with the local event hook:

```bash
sudo grep LIBRESPOT_ONEVENT /etc/raspotify/conf
journalctl -u raspotify -b -n 100 --no-pager
sudo cat /run/raspotify/now-playing.json
curl http://127.0.0.1:3000/api/current
```

If Spotify's cloud playback state gets stuck with an impossible timestamp,
restart the receiver and select the Pi again from Spotify:
